...
class CNN_LSTM_Model(object):
    ...
    def train_process(self):
        df1 = self.get_data(r"C:\Users\86178\Desktop\项目组\new\data\7_breach_temp.xlsx", 3)
        print("--------df1--------")
        print(df1)
        df2 = self.get_data(r"C:\Users\86178\Desktop\项目组\new\data\8_breach_temp.xlsx", 3)
        df3 = self.get_data(r"C:\Users\86178\Desktop\项目组\new\data\8_breach_temp.xlsx", 4)
        # df3 = self.get_data("C:\\Users\\TITAN3\\Desktop\\核电站系统流程框架\\温度数据\\小破口1.0_temp.xlsx", 2)
    ...
    def build_model(self):  # (1, 5, 1)
        ...
        model = Sequential()
        # model.add()
        model.add(Conv1D(activation='relu', input_shape=(self.step, 1), padding='same', filters=8, kernel_size=2))
    ...
    def show(self, Y_TEST, yhat, loss_ave):
        ...
        plt.xlabel('Time (s)')
        plt.ylabel('Normalized SG Break Temperature')
...
def main():
    ...
    X_train, Y_train = cnn_lstm.train_process()
    cnn_lstm.train(model, X_train, Y_train)
    Y_TEST, yhat, loss_ave = cnn_lstm.predict(model)
    cnn_lstm.show(Y_TEST, yhat, loss_ave)
